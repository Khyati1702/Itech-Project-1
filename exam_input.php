...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $exam = $_POST['exam']; // Specify which exam (e.g., Exam1, Exam2)
    $score = $_POST['score'];
    $comments = $_POST['comments'];

    // Validate inputs
    if (!empty($studentID) && !empty($exam) && !empty($score) && is_numeric($score)) {
        $query = $config->prepare("INSERT INTO exam_scores (StudentID, TeacherID, $exam, Comments_$exam) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE $exam = VALUES($exam), Comments_$exam = VALUES(Comments_$exam), ScoreTimestamp = NOW()");
        if (!$query) {
